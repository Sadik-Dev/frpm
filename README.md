...
- [Synthesizing perspectives](#synthesizing-perspectives---synthesize-missing)
- [Testing](#testing)
...
[Optional: local image generation](#optional-local-image-generation-frpmgenerate)
and [Synthesizing perspectives](#synthesizing-perspectives---synthesize-missing)
for details.
...
## Synthesizing perspectives (`--synthesize-missing`)

Real footage doesn't capture every useful angle - a talking-head video
might never show a clean profile shot, or never catch the person
laughing. This stage recreates a highly realistic image for pose/expression
categories from the same set the diversity selector targets (see
[Diversity selection](#how-it-works)), controlled by `--synthesis-mode`:

* **`missing` (default)** - only fills the specific gaps the source video
  didn't capture.
* **`full`** - recreates a **complete AI-reconstructed multi-perspective
  character set**: every target angle/expression (front, three-quarter
  left/right, profile left/right, looking up/down x neutral/smile/serious,
  plus laughing/talking/focused/confident) is generated, **regardless of
  whether real footage already covers it** - the person seen from every
  angle, ultra-realistic, all seeded from their own real photos.

```bash
pip install -e ".[generate]"

# Fill only the gaps the video didn't capture (fast, minimal changes):
python main.py "URL" --synthesize-missing

# Full ultra-realistic multi-perspective reconstruction of the identified
# person (up to 17 images: every pose x expression combination):
python main.py "URL" --synthesize-missing --synthesis-mode full
```

`--synthesis-mode full` automatically enables GFPGAN face-restoration +
2x upscale (512->1024px output) for maximum sharpness/realism, and lifts
the category cap so every target perspective is generated by default
(override either with `--no-face-restore` / `--synthesis-max-categories`).
Expect roughly 1-3 minutes per image on CPU once models are cached (several
GB of one-time model downloads on first use) - a full 17-category
reconstruction takes on the order of 30-60 minutes.

How it works: for each target category, the best *real* photo of the
...
profile) and processed with Stable Diffusion 1.5 img2img + IP-Adapter Plus
Face, seeded from that same photo for identity. Starting from a real photo
(rather than pure noise) keeps skin tone, hair, and lighting far more
consistent than plain text-to-image - this is what makes the perspective
reconstructions "the same person," not a generic AI face.
...
Flags:
* `--synthesis-mode {missing,full}` - which categories to (re)generate (default: `missing`)
* `--synthesis-count N` - images per category (default: 1)
...
* `--synthesis-steps N` - diffusion steps per image; higher is sharper but
  slower (default: 30)
* `--synthesis-max-categories N` - cap on categories synthesized per run, to
  bound CPU time (default: 6 for `missing` mode, all 17 for `full` mode)
* `--face-restore` / `--no-face-restore` - force GFPGAN face-restoration +
  2x upscale on or off (default: on automatically for `full` mode, off for
  `missing` mode)
...
GFPGAN's optional restoration pass then upscales back to 1024px with a
face-specialized network rather than a plain resize, verified on a real
run to produce noticeably sharper, more detailed final images.
